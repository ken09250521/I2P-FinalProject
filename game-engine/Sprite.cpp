...
namespace Engine {
	Sprite::Sprite(std::vector<std::string> imgPaths, float x, float y, float frameDuration, float w, float h, float anchorX, float anchorY,
		float rotation, float vx, float vy, unsigned char r, unsigned char g, unsigned char b, unsigned char a) :
		Image(imgPaths[AnimationType::leftMove][0], x, y, w, h, anchorX, anchorY), Rotation(rotation), Velocity(Point(vx, vy)), Tint(al_map_rgba(r, g, b, a)), frameDuration(frameDuration), frameTimer(0.0f), curFrames(0), currentAnimationType(AnimationType::Move) {
			for (const std::string &anim : imgPaths){
